...
#import "PGXResultSet.h"
#import "PGXDatabasePool.h"

NS_ASSUME_NONNULL_BEGIN

#if ! __has_feature(objc_arc)
    #define PGXDBAutorelease(__v) ([__v autorelease]);
    #define PGXDBReturnAutoreleased PGXDBAutorelease

    #define PGXDBRetain(__v) ([__v retain]);
    #define PGXDBReturnRetained PGXDBRetain

    #define PGXDBRelease(__v) ([__v release]);

    #define PGXDBDispatchQueueRelease(__v) (dispatch_release(__v));
#else
    // -fobjc-arc
    #define PGXDBAutorelease(__v)
    #define PGXDBReturnAutoreleased(__v) (__v)

    #define PGXDBRetain(__v)
    #define PGXDBReturnRetained(__v) (__v)

    #define PGXDBRelease(__v)

// If OS_OBJECT_USE_OBJC=1, then the dispatch objects will be treated like ObjC objects
// and will participate in ARC.
// See the section on "Dispatch Queues and Automatic Reference Counting" in "Grand Central Dispatch (GCD) Reference" for details. 
    #if OS_OBJECT_USE_OBJC
        #define PGXDBDispatchQueueRelease(__v)
    #else
        #define PGXDBDispatchQueueRelease(__v) (dispatch_release(__v));
    #endif
#endif
...
typedef int(^PGXDBExecuteStatementsCallbackBlock)(NSDictionary *resultsDictionary);

typedef NS_ENUM(int, PGXDBCheckpointMode) {
    PGXDBCheckpointModePassive  = 0, // SQLITE_CHECKPOINT_PASSIVE,
    PGXDBCheckpointModeFull     = 1, // SQLITE_CHECKPOINT_FULL,
    PGXDBCheckpointModeRestart  = 2, // SQLITE_CHECKPOINT_RESTART,
    PGXDBCheckpointModeTruncate = 3  // SQLITE_CHECKPOINT_TRUNCATE
};

/** A SQLite ([http://sqlite.org/](http://sqlite.org/)) Objective-C wrapper.
 
 ### Usage
 The three main classes in PGXDB are:

 - `PGXDatabase` - Represents a single SQLite database.  Used for executing SQL statements.
 - `<PGXResultSet>` - Represents the results of executing a query on an `PGXDatabase`.
 - `<PGXDatabaseQueue>` - If you want to perform queries and updates on multiple threads, you'll want to use this class.

 ### See also
 
 - `<PGXDatabasePool>` - A pool of `PGXDatabase` objects.
 - `<PGXStatement>` - A wrapper for `sqlite_stmt`.
 
 ### External links
 
 - [PGXDB on GitHub](https://github.com/ccgus/fmdb) including introductory documentation
 - [SQLite web site](http://sqlite.org/)
 - [PGXDB mailing list](http://groups.google.com/group/fmdb)
 - [SQLite FAQ](http://www.sqlite.org/faq.html)
 
 @warning Do not instantiate a single `PGXDatabase` object and use it across multiple threads. Instead, use `<PGXDatabaseQueue>`.
...
@interface PGXDatabase : NSObject
...
/** Create a `PGXDatabase` object.
 
 An `PGXDatabase` is created with a path to a SQLite database file.  This path can be one of these three:

 1. A file system path.  The file does not have to exist on disk.  If it does not exist, it is created for you.
 2. An empty string (`@""`).  An empty database is created at a temporary location.  This database is deleted with the `PGXDatabase` connection is closed.
 3. `nil`.  An in-memory database is created.  This database will be destroyed with the `PGXDatabase` connection is closed.

 For example, to create/open a database in your Mac OS X `tmp` folder:

    PGXDatabase *db = [PGXDatabase databaseWithPath:@"/tmp/tmp.db"];

 Or, in iOS, you might open a database in the app's `Documents` directory:

    NSString *docsPath = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES)[0];
    NSString *dbPath   = [docsPath stringByAppendingPathComponent:@"test.db"];
    PGXDatabase *db     = [PGXDatabase databaseWithPath:dbPath];
 ...
 @return `PGXDatabase` object if successful; `nil` if failure.
...
/** Create a `PGXDatabase` object.
 ...
 2. `nil`.  An in-memory database is created.  This database will be destroyed with the `PGXDatabase` connection is closed.
...
/** Initialize a `PGXDatabase` object.
...
/** Initialize a `PGXDatabase` object.
 ...
 An `PGXDatabase` is created with a local file URL to a SQLite database file.  This path can be one of these three:
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(__attribute__((noescape)) PGXDBExecuteStatementsCallbackBlock _Nullable)block;
...
/** Execute select statement
 ...
 Executing queries returns an `<PGXResultSet>` object if successful, and `nil` upon failure.  Like executing updates, there is a variant that accepts an `NSError **` parameter.  Otherwise you should use the `<lastErrorMessage>` and `<lastErrorMessage>` methods to determine why a query failed.
 
 In order to iterate through the results of your query, you use a `while()` loop.  You also need to "step" (via `<[PGXResultSet next]>`) from one record to the other.
 ...
 @return A `<PGXResultSet>` for the result set upon success; `nil` upon failure. If failed, you can call `<lastError>`, `<lastErrorCode>`, or `<lastErrorMessage>` for diagnostic information regarding the failure.
 
 @see PGXResultSet
 @see [`PGXResultSet next`](<[PGXResultSet next]>)
 @see [`sqlite3_bind`](http://sqlite.org/c3ref/bind_blob.html)
...
- (PGXResultSet * _Nullable)executeQuery:(NSString*)sql, ...;
...
- (PGXResultSet * _Nullable)executeQueryWithFormat:(NSString*)format, ... NS_FORMAT_FUNCTION(1,2);
...
- (PGXResultSet * _Nullable)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments;
...
/** Execute select statement
 ...
 `func executeQuery(sql: String, values: [Any]?) throws  -> PGXResultSet!`
...
- (PGXResultSet * _Nullable)executeQuery:(NSString *)sql values:(NSArray * _Nullable)values error:(NSError * _Nullable __autoreleasing *)error;
...
- (PGXResultSet * _Nullable)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary * _Nullable)arguments;


// Documentation forthcoming.
- (PGXResultSet * _Nullable)executeQuery:(NSString *)sql withVAList:(va_list)args;
...
/** Begin a transaction
 ...
 @warning    Unlike SQLite's `BEGIN TRANSACTION`, this method currently performs
             ...
             is likely to change in future versions of PGXDB, whereby this method
 ...
 */
- (BOOL)checkpoint:(PGXDBCheckpointMode)checkpointMode error:(NSError * _Nullable *)error;
...
/** Performs a WAL checkpoint
 ...
 @return YES on success, otherwise NO.
 */
- (BOOL)checkpoint:(PGXDBCheckpointMode)checkpointMode name:(NSString * _Nullable)name error:(NSError * _Nullable *)error;
...
/** Performs a WAL checkpoint
 ...
 @return YES on success, otherwise NO.
 */
- (BOOL)checkpoint:(PGXDBCheckpointMode)checkpointMode name:(NSString * _Nullable)name logFrameCount:(int * _Nullable)logFrameCount checkpointCount:(int * _Nullable)checkpointCount error:(NSError * _Nullable *)error;
...
/** Adds SQL functions or aggregates or to redefine the behavior of existing SQL functions or aggregates.
 ...
 For example:
    ...
    PGXResultSet *rs = [db executeQuery:@"SELECT * FROM employees WHERE RemoveDiacritics(first_name) LIKE 'jose'"];
...
/** Generate an `NSDateFormatter` that won't be broken by permutations of timezones or locales.
 ...
 Example:
    ...
    myDB.dateFormat = [PGXDatabase storeableDateFormat:@"yyyy-MM-dd HH:mm:ss"];
 ...
 @warning Note that `NSDateFormatter` is not thread-safe, so the formatter generated by this method should be assigned to only one PGXDB instance and should not be used for other purposes.
...
/** Set to a date formatter to use string dates with sqlite instead of the default UNIX timestamps.
 ...
 @param format Set to nil to use UNIX timestamps. Defaults to nil. Should be set using a formatter generated using PGXDatabase::storeableDateFormat.
 ...
 @warning Note there is no direct getter for the `NSDateFormatter`, and you should not use the formatter you pass to PGXDB for other purposes, as `NSDateFormatter` is not thread-safe.
 */
...
/** Objective-C wrapper for `sqlite3_stmt`
 ...
 This is a wrapper for a SQLite `sqlite3_stmt`. Generally when using PGXDB you will not need to interact directly with `PGXStatement`, but rather with `<PGXDatabase>` and `<PGXResultSet>` only.
...
@interface PGXStatement : NSObject {
    void *_statement;
    NSString *_query;
    long _useCount;
    BOOL _inUse;
