...
+ (instancetype)resultSetWithStatement:(PGXStatement *)statement usingParentDatabase:(PGXDatabase*)aDB {
    ...
    return PGXDBReturnAutoreleased(rs);
}
...
- (void)dealloc {
    ...
    PGXDBRelease(_columnNameToIndexMap);
...
- (NSDictionary *)resultDict {
    ...
    if (num_cols > 0) {
        ...
        return PGXDBReturnAutoreleased([dict copy]);
    }
    else {
    ...
    if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
        ...
        if (outErr) {
            if (_parentDB) {
                ...
                *outErr = [NSError errorWithDomain:@"PGXDatabase" code:SQLITE_MISUSE userInfo:errorMessage];
            }
