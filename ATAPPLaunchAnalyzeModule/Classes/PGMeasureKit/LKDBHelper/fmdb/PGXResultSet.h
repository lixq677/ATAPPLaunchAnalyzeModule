...
/** Represents the results of executing a query on an `<PGXDatabase>`.
...
@interface PGXResultSet : NSObject

@property (nonatomic, retain, nullable) PGXDatabase *parentDB;
...
/** `PGXStatement` used by result set. */

@property (atomic, retain, nullable) PGXStatement *statement;

///------------------------------------
/// @name Creating and closing a result set
///------------------------------------

/** Create result set from `<PGXStatement>`
 
 @param statement A `<PGXStatement>` to be performed
 
 @param aDB A `<PGXDatabase>` to be used
 
 @return A `PGXResultSet` on success; `nil` on failure
 */

+ (instancetype)resultSetWithStatement:(PGXStatement *)statement usingParentDatabase:(PGXDatabase*)aDB;
