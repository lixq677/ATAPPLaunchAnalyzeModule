...
/** To perform queries and updates on multiple threads, you'll want to use `PGXDatabaseQueue`.

 Using a single instance of `<PGXDatabase>` from multiple threads at once is a bad idea.  It has always been OK to make a `<PGXDatabase>` object *per thread*.  Just don't share a single instance across threads, and definitely not across multiple threads at the same time.

 Instead, use `PGXDatabaseQueue`. Here's how to use it:

 First, make your queue.

    PGXDatabaseQueue *queue = [PGXDatabaseQueue databaseQueueWithPath:aPath];

 Then use it like so:

    [queue inDatabase:^(PGXDatabase *db) {
        [db executeUpdate:@"INSERT INTO myTable VALUES (?)", [NSNumber numberWithInt:1]];
        [db executeUpdate:@"INSERT INTO myTable VALUES (?)", [NSNumber numberWithInt:2]];
        [db executeUpdate:@"INSERT INTO myTable VALUES (?)", [NSNumber numberWithInt:3]];

        PGXResultSet *rs = [db executeQuery:@"select * from foo"];
        while ([rs next]) {
            //…
        }
    }];
 ...
 An easy way to wrap things up in a transaction can be done like this:
    ...
    [queue inTransaction:^(PGXDatabase *db, BOOL *rollback) {
        [db executeUpdate:@"INSERT INTO myTable VALUES (?)", [NSNumber numberWithInt:1]];
        [db executeUpdate:@"INSERT INTO myTable VALUES (?)", [NSNumber numberWithInt:2]];
        [db executeUpdate:@"INSERT INTO myTable VALUES (?)", [NSNumber numberWithInt:3]];
 ...
 `PGXDatabaseQueue` will run the blocks on a serialized queue (hence the name of the class).  So if you call `PGXDatabaseQueue`'s methods from multiple threads at the same time, they will be executed in the order they are received.  This way queries and updates won't step on each other's toes, and every one is happy.

 ### See also

 - `<PGXDatabase>`

 @warning Do not instantiate a single `<PGXDatabase>` object and use it across multiple threads. Use `PGXDatabaseQueue` instead.
 
 @warning The calls to `PGXDatabaseQueue`'s methods are blocking.  So even though you are passing along blocks, they will **not** be run on another thread.

 */

@interface PGXDatabaseQueue : NSObject
/** Path of database */
...
/** Create queue using path.
 ...
 @return The `PGXDatabaseQueue` object. `nil` on error.
 */
...
/** Returns the Class of 'PGXDatabase' subclass, that will be used to instantiate database object.
 
 Subclasses can override this method to return specified Class of 'PGXDatabase' subclass.
 
 @return The Class of 'PGXDatabase' subclass, that will be used to instantiate database object.
 */
...
/** Synchronously perform database operations on queue.
 ...
 @param block The code to be run on the queue of `PGXDatabaseQueue`
 */
...
// If you need to nest, use PGXDatabase's startSavePointWithName:error: instead.
- (NSError * _Nullable)inSavePoint:(__attribute__((noescape)) void (^)(PGXDatabase *db, BOOL *rollback))block;
