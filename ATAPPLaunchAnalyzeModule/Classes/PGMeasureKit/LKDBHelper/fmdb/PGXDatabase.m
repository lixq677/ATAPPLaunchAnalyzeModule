...
- (PGXResultSet * _Nullable)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray * _Nullable)arrayArgs orDictionary:(NSDictionary * _Nullable)dictionaryArgs orVAList:(va_list)args;
...
#pragma mark PGXDatabase instantiation and deallocation
...
+ (instancetype)databaseWithURL:(NSURL *)url {
    return PGXDBReturnAutoreleased([[self alloc] initWithURL:url]);
}
...
- (void)dealloc {
    [self close];
    PGXDBRelease(_openResultSets);
    PGXDBRelease(_cachedStatements);
    PGXDBRelease(_dateFormat);
    PGXDBRelease(_databasePath);
    PGXDBRelease(_openFunctions);
...
// /* need to make sure to do X with PGXDB version 2.4 or later */
// if ([PGXDatabase PGXDBVersion] >= 0x0240) { … }

+ (SInt32)PGXDBVersion {
    
    // we go through these hoops so that we only have to change the version number in a single spot.
    static dispatch_once_t once;
    static SInt32 PGXDBVersionVal = 0;
    
    dispatch_once(&once, ^{
        NSString *prodVersion = [self PGXDBUserVersion];
        ...
        char *e = nil;
        PGXDBVersionVal = (int) strtoul([junk UTF8String], &e, 16);
        
    });
    
    return PGXDBVersionVal;
}
...
static int PGXDBDatabaseBusyHandler(void *f, int count) {
    PGXDatabase *self = (__bridge PGXDatabase*)f;
...
- (void)setMaxBusyRetryTimeInterval:(NSTimeInterval)timeout {
    ...
    if (timeout > 0) {
        sqlite3_busy_handler(_db, &PGXDBDatabaseBusyHandler, (__bridge void *)(self));
    }
    else {
...
// but for folks who don't bother noticing that the interface to PGXDatabase changed,
// we'll still implement the method so they don't get suprise crashes
- (int)busyRetryTimeout {
    NSLog(@"%s:%d", __FUNCTION__, __LINE__);
    NSLog(@"PGXDB: busyRetryTimeout no longer works, please use maxBusyRetryTimeInterval");
    return -1;
}
...
#pragma unused(i)
    NSLog(@"%s:%d", __FUNCTION__, __LINE__);
    NSLog(@"PGXDB: setBusyRetryTimeout does nothing, please use setMaxBusyRetryTimeInterval:");
}
...
- (void)closeOpenResultSets {
    ...
    NSSet *openSetCopy = PGXDBReturnAutoreleased([_openResultSets copy]);
...
- (void)resultSetDidClose:(PGXResultSet *)resultSet {
...
- (void)clearCachedStatements {
    ...
    for (NSMutableSet *statements in [_cachedStatements objectEnumerator]) {
        for (PGXStatement *statement in [statements allObjects]) {
            [statement close];
        }
    }
...
- (PGXStatement*)cachedStatementForQuery:(NSString*)query {
    
    NSMutableSet* statements = [_cachedStatements objectForKey:query];
    
    return [[statements objectsPassingTest:^BOOL(PGXStatement* statement, BOOL *stop) {
...
- (void)setCachedStatement:(PGXStatement*)statement forQuery:(NSString*)query {
    NSParameterAssert(query);
    if (!query) {
        NSLog(@"API misuse, -[PGXDatabase setCachedStatement:forQuery:] query must not be nil");
        return;
    }
...
+ (NSDateFormatter *)storeableDateFormat:(NSString *)format {
    ...
    NSDateFormatter *result = PGXDBReturnAutoreleased([[NSDateFormatter alloc] init]);
    result.dateFormat = format;
    result.timeZone = [NSTimeZone timeZoneForSecondsFromGMT:0];
    result.locale = PGXDBReturnAutoreleased([[NSLocale alloc] initWithLocaleIdentifier:@"en_US"]);
    return result;
}
...
- (void)setDateFormat:(NSDateFormatter *)format {
    PGXDBAutorelease(_dateFormat);
    _dateFormat = PGXDBReturnRetained(format);
}
...
- (BOOL)goodConnection {
    ...
    PGXResultSet *rs = [self executeQuery:@"select name from sqlite_master where type='table'"];
...
- (void)warnInUse {
    NSLog(@"The PGXDatabase %@ is currently in use.", self);
    
#ifndef NS_BLOCK_ASSERTIONS
    if (_crashOnErrors) {
        NSAssert(false, @"The PGXDatabase %@ is currently in use.", self);
        abort();
    }
#endif
}
...
- (BOOL)databaseExists {
    ...
    if (!_isOpen) {
        ...
        NSLog(@"The PGXDatabase %@ is not open.", self);
        
#ifndef NS_BLOCK_ASSERTIONS
        if (_crashOnErrors) {
            NSAssert(false, @"The PGXDatabase %@ is not open.", self);
            abort();
        }
#endif
...
- (NSError*)errorWithMessage:(NSString *)message {
    ...
    return [NSError errorWithDomain:@"PGXDatabase" code:sqlite3_errcode(_db) userInfo:errorMessage];
}
...
- (PGXResultSet *)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary *)arguments {
    return [self executeQuery:sql withArgumentsInArray:nil orDictionary:arguments orVAList:nil];
}

- (PGXResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    PGXStatement *statement  = 0x00;
    PGXResultSet *rs         = 0x00;
    ...
    PGXDBRetain(statement); // to balance the release below
    
    if (!statement) {
        statement = [[PGXStatement alloc] init];
    ...
    rs = [PGXResultSet resultSetWithStatement:statement usingParentDatabase:self];
    [rs setQuery:sql];
...
- (PGXResultSet *)executeQuery:(NSString*)sql, ... {
    va_list args;
    va_start(args, sql);
...
- (PGXResultSet *)executeQueryWithFormat:(NSString*)format, ... {
    va_list args;
...
- (PGXResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments {
    return [self executeQuery:sql withArgumentsInArray:arguments orDictionary:nil orVAList:nil];
}

- (PGXResultSet *)executeQuery:(NSString *)sql values:(NSArray *)values error:(NSError * __autoreleasing *)error {
    PGXResultSet *rs = [self executeQuery:sql withArgumentsInArray:values orDictionary:nil orVAList:nil];
    if (!rs && error) {
...
- (PGXResultSet *)executeQuery:(NSString*)sql withVAList:(va_list)args {
...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    if (_shouldCacheStatements && !cachedStmt) {
        cachedStmt = [[PGXStatement alloc] init];
        
        [cachedStmt setStatement:pStmt];
        
        [self setCachedStatement:cachedStmt forQuery:sql];
        
        PGXDBRelease(cachedStmt);
    }
...
int PGXDBExecuteBulkSQLCallback(void *theBlockAsVoid, int columns, char **values, char **names); // shhh clang.
int PGXDBExecuteBulkSQLCallback(void *theBlockAsVoid, int columns, char **values, char **names) {
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(__attribute__((noescape)) PGXDBExecuteStatementsCallbackBlock)block {
    
    int rc;
    char *errmsg = nil;
    
    rc = sqlite3_exec([self sqliteHandle], [sql UTF8String], block ? PGXDBExecuteBulkSQLCallback : nil, (__bridge void *)(block), &errmsg);
...
static NSString *PGXDBEscapeSavePointName(NSString *savepointName) {
...
- (BOOL)startSavePointWithName:(NSString*)name error:(NSError**)outErr {
    ...
    NSString *sql = [NSString stringWithFormat:@"savepoint '%@';", PGXDBEscapeSavePointName(name)];
...
- (BOOL)releaseSavePointWithName:(NSString*)name error:(NSError**)outErr {
    ...
    NSString *sql = [NSString stringWithFormat:@"release savepoint '%@';", PGXDBEscapeSavePointName(name)];
...
- (BOOL)rollbackToSavePointWithName:(NSString*)name error:(NSError**)outErr {
    ...
    NSString *sql = [NSString stringWithFormat:@"rollback transaction to savepoint '%@';", PGXDBEscapeSavePointName(name)];
...
- (BOOL)checkpoint:(PGXDBCheckpointMode)checkpointMode error:(NSError * __autoreleasing *)error {
    return [self checkpoint:checkpointMode name:nil logFrameCount:NULL checkpointCount:NULL error:error];
}

- (BOOL)checkpoint:(PGXDBCheckpointMode)checkpointMode name:(NSString *)name error:(NSError * __autoreleasing *)error {
    return [self checkpoint:checkpointMode name:name logFrameCount:NULL checkpointCount:NULL error:error];
}

- (BOOL)checkpoint:(PGXDBCheckpointMode)checkpointMode name:(NSString *)name logFrameCount:(int *)logFrameCount checkpointCount:(int *)checkpointCount error:(NSError * __autoreleasing *)error
{
...
void PGXDBBlockSQLiteCallBackFunction(sqlite3_context *context, int argc, sqlite3_value **argv); // -Wmissing-prototypes
void PGXDBBlockSQLiteCallBackFunction(sqlite3_context *context, int argc, sqlite3_value **argv) {
#if ! __has_feature(objc_arc)
...
- (void)makeFunctionNamed:(NSString *)name arguments:(int)arguments block:(void (^)(void *context, int argc, void **argv))block {
    ...
    id b = PGXDBReturnAutoreleased([block copy]);
    
    [_openFunctions addObject:b];
    
    /* I tried adding custom functions to release the block when the connection is destroyed- but they seemed to never be called, so we use _openFunctions to store the values instead. */
#if ! __has_feature(objc_arc)
    sqlite3_create_function([self sqliteHandle], [name UTF8String], arguments, SQLITE_UTF8, (void*)b, &PGXDBBlockSQLiteCallBackFunction, 0x00, 0x00);
#else
    sqlite3_create_function([self sqliteHandle], [name UTF8String], arguments, SQLITE_UTF8, (__bridge void*)b, &PGXDBBlockSQLiteCallBackFunction, 0x00, 0x00);
#endif
}
