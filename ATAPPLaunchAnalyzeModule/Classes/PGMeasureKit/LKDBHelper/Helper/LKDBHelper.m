...
@property (nonatomic, weak) PGXDatabase *usingdb;
@property (nonatomic, strong) PGXDatabaseQueue *bindingQueue;
...
+ (void)setLogError:(BOOL)logError {
    ...
    @synchronized(dbArray) {
        [dbArray enumerateObjectsUsingBlock:^(LKDBWeakObject *weakObj, NSUInteger idx, BOOL *stop) {
            [weakObj.obj executeDB:^(PGXDatabase *db) {
...
- (void)openDB {
    ...
    self.bindingQueue = [[PGXDatabaseQueue alloc] initWithPath:filePath
                                                        flags:LKDBOpenFlags];
    [self.bindingQueue inDatabase:^(PGXDatabase *db) {
        db.logsErrors = LKDBLogErrorEnable;
    }];
...
#pragma mark - core
- (void)executeDB:(void (^)(PGXDatabase *db))block {
    if (!block) {
...
- (void)dropAllTable {
    [self executeDB:^(PGXDatabase *db) {
        PGXResultSet *set = [db executeQuery:@"select name from sqlite_master where type='table'"];
...
- (void)fixSqlColumnsWithClass:(Class)clazz tableName:(NSString *)tableName {
    [self executeDB:^(PGXDatabase *db) {
        ...
        PGXResultSet *set = [db executeQuery:select];
...
- (BOOL)getTableCreatedWithTableName:(NSString *)tableName {
    ...
    [self executeDB:^(PGXDatabase *db) {
        PGXResultSet *set = [db executeQuery:@"select count(name) from sqlite_master where type='table' and name=?", tableName];
        if ([set next]) {
...
- (NSMutableArray *)searchWithRAWSQL:(NSString *)sql toClass:(Class)modelClass {
    __block NSMutableArray *results = nil;
    [self executeDB:^(PGXDatabase *db) {
        PGXResultSet *set = [db executeQuery:sql];
...
- (NSMutableArray *)search:(Class)modelClass withSQL:(NSString *)sql, ... {
    ...
    __block NSMutableArray *results = nil;
    [self executeDB:^(PGXDatabase *db) {
        PGXResultSet *set = [db executeQuery:sql withVAList:*argsPoint];
        results = [self executeResult:set Class:modelClass tableName:nil];
        [set close];
    }];
...
- (NSMutableArray *)executeOneColumnResult:(PGXResultSet *)set {
...
- (void)foreachResultSet:(PGXResultSet *)set block:(void(^)(void))block {
    while ([set next]) {
        [self inAutoReleaseExecuteBlock:block];
    }
}

- (NSMutableArray *)executeResult:(PGXResultSet *)set Class:(Class)modelClass tableName:(NSString *)tableName {
    NSMutableArray *array = [NSMutableArray arrayWithCapacity:0];
    if (!modelClass) {
...
+ (void)clearFileWithTable:(Class)modelClass columns:(NSArray *)columns type:(NSInteger)type {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
        ...
        [[modelClass getUsingLKDBHelper] executeDB:^(PGXDatabase *db) {
            NSMutableArray *tempfiles = [NSMutableArray arrayWithCapacity:6];
            PGXResultSet *set = [db executeQuery:querySql];
